...
export class WebAudioFontTicker {
  ...
  playLoop(
    ...
    this.startTicks(
      audioContext,
      function (when: number, from: number, to: number) {
...
export class WebAudioFontLoader {
  ...
  decodeAfterLoading(audioContext: AudioContext, variableName: string) {
    ...
    this.waitOrFinish(variableName, function () {
      me.player.adjustPreset(
        audioContext,
        window[variableName] as any as WavePreset
      );
    });
  }
...
export class WebAudioFontPlayer {
  ...
  queueChord(
    ...
    for (var i = 0; i < pitches.length; i++) {
      ...
      var envlp: WaveEnvelope | null = this.queueWaveTable(
        audioContext,
        target,
        preset,
        when,
  ...
  setupEnvelope(
    ...
    (envelope as any as GainNode).gain.setValueAtTime(
      this.noZeroVolume(0),
      audioContext.currentTime
    );
    var lastTime = 0;
    ...
    for (var i = 0; i < ahdsr.length; i++) {
      if (ahdsr[i].duration > 0) {
        if (ahdsr[i].duration + lastTime > duration) {
          ...
          (envelope as any as GainNode).gain.linearRampToValueAtTime(
  ...
  cancelQueue(audioContext: AudioContext) {
    for (var i = 0; i < this.envelopes.length; i++) {
      ...
      (e as any as GainNode).gain.setValueAtTime(
        this.nearZero,
        audioContext.currentTime
      );
      e.when = -1;
